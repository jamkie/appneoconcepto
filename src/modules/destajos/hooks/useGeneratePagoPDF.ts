...
export const useGeneratePagoPDF = () => {
  ...
  const generatePDF = async (pago: PagoWithDetails) => {
    const doc = new jsPDF({
    ...
    const primaryColor: [number, number, number] = [76, 107, 125]; // hsl(200, 25%, 35%) approx
    ...
    drawField('Nombre', pago.instaladores?.nombre || 'N/A', margin, yPos, halfWidth);
    drawField('Banco', pago.instaladores?.nombre_banco || 'No especificado', margin + halfWidth + 10, yPos, halfWidth);
    
    yPos += 18;
    drawField('Número de Cuenta / CLABE', pago.instaladores?.numero_cuenta || 'No registrado', margin, yPos, pageWidth - margin * 2);
    ...
    drawField('Fecha del Pago', format(new Date(pago.fecha), "dd/MM/yyyy", { locale: es }), margin, yPos, halfWidth);
    drawField('Método de Pago', paymentMethodLabels[pago.metodo_pago], margin + halfWidth + 10, yPos, halfWidth);
    
    yPos += 18;
    drawField('Obra / Proyecto', pago.obras?.nombre || 'N/A', margin, yPos, halfWidth);
    if (pago.referencia) {
      drawField('Referencia', pago.referencia, margin + halfWidth + 10, yPos, halfWidth);
    }
    ...
    // Save the PDF
    const filename = `Comprobante_Pago_${pago.id.slice(0, 8)}_${format(new Date(pago.fecha), 'yyyyMMdd')}.pdf`;
    doc.save(filename);
