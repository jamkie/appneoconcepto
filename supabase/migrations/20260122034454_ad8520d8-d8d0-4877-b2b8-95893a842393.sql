-- Add manual closure fields to obras table
ALTER TABLE public.obras 
ADD COLUMN IF NOT EXISTS cerrado_manualmente boolean NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS motivo_cierre text;

-- Update trigger to respect manual closures
...
BEGIN
  ...
  -- Get current obra state, discount, and manual closure flag
  SELECT estado, descuento, cerrado_manualmente INTO v_obra_estado, v_descuento, v_cerrado_manualmente
  ...
  ELSIF NOT v_should_be_closed AND v_obra_estado = 'cerrada' THEN
    -- Only reopen if it wasn't manually closed
    IF NOT COALESCE(v_cerrado_manualmente, false) THEN
